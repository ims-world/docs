...
<Info>
**LABRAX** (Révision Core IMS-01) est le rack serveur physique 3D-printé qui héberge l'ensemble du matériel informatique IMS-WORLD. Cette page documente la vue générale du châssis, l'inventaire physique **étage par étage**, la distribution électrique et le câblage.
</Info>
...
| Étage / Slot | Équipement Physique Encastré | Connectivité & Ports | Fiche Détaillée (Vérité Terrain) |
|---|---|---|---|
| **Étage 1 (Module 2U)** | Module 2U avec écran Wisecoco 7.84" LCD, OLED 0.91", bouton GPIO & RPi 3B+ | Port Switch #4 (100M) • IP `100.64.0.12` | 📺 [Raspberry Pi Kiosk](/infrastructure/rpi-monitor) |
| **Étage 2** | Mac Mini 2014 (Slot Standby chaud) | Port Switch #3 (1G) • IP `100.64.0.7` | 🍎 [Mac Mini 2014](/infrastructure/mac-mini) |
| **Étage 3** | Switch NETGEAR GS308EV4 (8p Gérés) + Patch Panel RJ45 12p | Ports 1 à 4 câblés • Uplink Bbox | 🔌 *(Voir câblage ci-dessous)* |
| **Étage 4** | Minisforum MS-01 (Carte SATA PCIe Tbest ASM1166 6 ports) | Port Switch #2 (2.5G) • IP `100.64.0.9` | 💻 [Host Proxmox (MS-01)](/infrastructure/proxmox-host) |
| **Étage 5** | 4-Pack Hard Drive Tray Caddy 3.5" Dell (HDD Seagate 3To) | Nappe SATA via carte PCIe MS-01 | 🗄️ [IMS-NAS (LXC 100)](/infrastructure/ims-nas) |
...
graph TB
    ...
    subgraph SWITCH ["🌐 Étage 3 — Switch NETGEAR GS308EV4"]
        NETGEAR["Switch NETGEAR 8 Ports"]
        BBOX["Uplink Routeur BBox (Port 1)"]
        BBOX <--> NETGEAR
    end

    subgraph SLOTS ["🗄️ Rack LABRAX (Inventaire Étage par Étage)"]
        RPI["Étage 1: Module 2U RPi Kiosk (Port 4 — 100.64.0.12)"]
        MAC_MINI["Étage 2: Mac Mini Standby (Port 3 — 100.64.0.7)"]
        MS01["Étage 4: Minisforum MS-01 (Port 2 — 100.64.0.9)"]
        HDD_DISKS["Étage 5: Caddies 3.5'' Dell (HDD 3To Seagate)"]
    end
