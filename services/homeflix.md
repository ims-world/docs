...
graph TB
    ...
    classDef pub fill:#0F6E56,stroke:#16A085,color:#fff;
...
<Warning>
**Cause réelle** : `WebUI\HostHeaderValidation` de qBittorrent (activé par défaut) rejette les requêtes dont le header `Host` ne correspond pas à un domaine autorisé — malgré un `ServerDomains=*` (wildcard) déjà en place, insuffisant pour une raison non totalement élucidée.
...
<Card title="Passthrough GPU (Iris Xe)" icon="microchip">
